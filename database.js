...
function initDatabase() {
  ...
  db.run(
    ...
    () => {
      // Insertar libros iniciales si hay menos de 26
      db.get("SELECT COUNT(*) as count FROM libros", (err, row) => {
        if (err) return console.error("❌ Error verificando libros:", err);
        if (!row || row.count < 26) {
...
function insertarLibrosIniciales() {
  ...
  const stmt = db.prepare(
    "INSERT OR REPLACE INTO libros (id, titulo, autor, genero, copias, img, sinopsis) VALUES (?, ?, ?, ?, ?, ?, ?)"
  );
