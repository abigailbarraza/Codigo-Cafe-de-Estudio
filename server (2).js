...
app.post("/register", (req, res) => {
  ...
  users.push(user);
  res.json({ status: "success", message: "Registrado con éxito", user });
});
...
app.post("/login", (req, res) => {
  ...
  else res.json({ status: "error", message: "Credenciales incorrectas" });
});
...
app.post("/reservas", (req, res) => {
  ...
  res.json({ success: true, reserva: req.body });
});

app.get("/reservas/:email", (req, res) => {
  res.json(reservas.filter(r => r.usuario === req.params.email));
});
...
app.post("/alquileres", (req, res) => {
  ...
  if (!libro) return res.json({ success: false, message: "Libro no encontrado" });

  const nuevo = { idLibro, usuario, fechaDev, titulo: libro.titulo, devuelto: false };
  alquileres.push(nuevo);

  res.json({ success: true, alquiler: nuevo });
});

app.get("/alquileres/:email", (req, res) => {
  res.json(alquileres.filter(a => a.usuario === req.params.email));
});
